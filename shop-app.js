...
// =============================================
const BACKEND_URL =
  "https://script.google.com/macros/s/AKfycbz1zkTHlVpnFgbMlscbjgGHXDRwhoAqYQeasInpWUDzn6dzC2aFC_DEykj_itklCHILRA/exec";
...
function sessionSet(token, name, email, role) {
  ...
  var bf = document.getElementById("btnFavorites");
...
function sessionGet() {
  if (!_session) {
    try {
      var s = localStorage.getItem("barsclusive_customer_session");
...
function sessionClear() {
  _session = null;
  try {
    localStorage.removeItem("barsclusive_customer_session");
  } catch (e) {}
  document.getElementById("userBtn").textContent = "👤 Login";
  document.getElementById("btnOrders").style.display = "none";
  var bf2 = document.getElementById("btnFavorites");
  if (bf2) bf2.style.display = "none";
  _favorites = [];
...
function isValidCoord(v) {
  return typeof v === "number" && isFinite(v) && Math.abs(v) > 0;
}
function saveLocationState() {
  try {
    localStorage.setItem(
      "barsclusive_shop_location",
      JSON.stringify(_locationState),
    );
  } catch (e) {}
}
function restoreLocationState() {
  try {
    ...
    if (parsed.label && document.getElementById("locationInput"))
      document.getElementById("locationInput").value = parsed.label;
...
function updateLocationUi() {
  ...
  var hasLocation = !!(
    filters.city ||
    _locationState.label ||
    (isValidCoord(Number(_userLat)) && isValidCoord(Number(_userLng)))
  );
  ...
  if (inputEl && _locationState.label && document.activeElement !== inputEl)
    inputEl.value = _locationState.label;
}
function setFilter(type, val, btn) {
  ...
  if (attr)
    document
      .querySelectorAll(".filter-btn[" + attr + "]")
...
function setCustomDate(val) {
  ...
  document
    .querySelectorAll(".filter-btn[data-filter-date]")
    .forEach((b) => b.classList.remove("active"));
  renderDeals();
}
async function geocodeLocationQuery(query) {
  ...
  try {
    var url =
      "https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit=6&countrycodes=ch&q=" +
...
function mapLocationResult(item) {
  ...
  var title =
    item.display_name ||
    [a.road, a.house_number, zip, city].filter(Boolean).join(", ");
  return {
...
async function applyLocation() {
  ...
  if (results.length) {
    ...
    _locationState.lat = null;
    _locationState.lng = null;
    _userLat = null;
    _userLng = null;
    saveLocationState();
    updateLocationUi();
...
function attachDistances() {
  ...
  allDeals.forEach(function (d) {
    var lat = Number(d.bar_lat),
      lng = Number(d.bar_lng);
    if (hasCoords && isValidCoord(lat) && isValidCoord(lng))
      d._dist = haversine(Number(_userLat), Number(_userLng), lat, lng);
    else delete d._dist;
  });
}
...
function getVisibleDeals() {
  ...
  var visible = allDeals.filter(function (d) {
    if (filters.cat !== "all" && !(d.categories || []).includes(filters.cat))
      return false;
...
try {
  ...
  if (_stored) {
    ...
    if (_parsed.data && Date.now() - _parsed.timestamp < 30 * 60 * 1000) {
      ...
      setTimeout(function () {
        var ld = document.getElementById("dealsLoading");
        if (ld) ld.style.display = "none";
        var dl = document.getElementById("dealsList");
...
async function loadDeals(forceRefresh = false) {
  ...
  if (
    !forceRefresh &&
    dealsCache.data &&
    now - dealsCache.timestamp < CACHE_DURATION
  ) {
  ...
  try {
    ...
    if (d.success) {
      ...
      try {
        localStorage.setItem(
          "barsclusive_deals_cache",
          JSON.stringify(dealsCache),
        );
      } catch (e) {}
      var ld = document.getElementById("dealsLoading");
      if (ld) ld.style.display = "none";
...
function matchTime(deal) {
  ...
  if (filters.time === "lunch")
    return (fromH <= 11 && toH >= 14) || (fromH >= 11 && fromH <= 14);
  if (filters.time === "evening")
    return (fromH <= 17 && toH >= 23) || (fromH >= 17 && fromH <= 23);
  return true;
}
...
function renderDeals() {
  ...
  if (!visible.length) {
    ...
    div.style.cssText =
      "grid-column:1/-1;text-align:center;padding:70px 20px;color:#666";
    div.innerHTML =
      '<div style="font-size:56px;margin-bottom:10px">🍸</div><div style="font-size:18px;font-weight:700;margin-bottom:6px">' +
      escHtml(shopT("keineDeals") || "Keine Deals gefunden") +
      '</div><div style="font-size:14px">' +
      escHtml(shopT("andereFilter") || "Versuche andere Filter") +
      "</div>";
    el.appendChild(div);
  ...
  visible.forEach(function (deal) {
    el.appendChild(buildDealCard(deal));
  });
  if (_shopMap && _mapView) updateShopMapMarkers();
}
...
function buildDealCard(deal) {
  ...
  const CAT_NAME = {
    ...
    dinner: "Dinner",
    events: "Events",
    pauschalgutscheine: "Rabatt-Gutschein",
  };
  ...
  if (deal.image_url) {
    ...
    if (imgUrl.indexOf("lh3.googleusercontent.com/d/") >= 0)
      gid = imgUrl.split("/d/")[1].split("=")[0];
    else if (imgUrl.indexOf("thumbnail?id=") >= 0)
      gid = imgUrl.split("id=")[1].split("&")[0];
    else if (imgUrl.indexOf("/d/") >= 0)
      gid = (imgUrl.split("/d/")[1] || "").split("/")[0];
    else if (imgUrl.indexOf("uc?") >= 0 && imgUrl.indexOf("id=") >= 0)
      gid = imgUrl.split("id=")[1].split("&")[0];
    ...
    img.onerror = function () {
      if (gid && !this.dataset.retried) {
        ...
        this.parentElement.textContent = isPauschal
          ? "\ud83c\udff7\ufe0f"
          : "\ud83c\udf79";
  ...
  if (mainCat) {
    ...
    b.textContent =
      (CAT_EMOJI[mainCat] || "") + " " + (CAT_NAME[mainCat] || mainCat);
    imgDiv.appendChild(b);
  }
  ...
  bar.textContent =
    deal.bar_name + (deal.bar_city ? " · " + deal.bar_city : "");
  ...
  if (deal.validity_type === "single" && deal.valid_single_date) {
    ...
    if (dateParts) {
      ...
      validity.textContent =
        "📅 Nur am " +
        d +
        (deal.valid_from_time && deal.valid_to_time
          ? " · " + deal.valid_from_time + "–" + deal.valid_to_time
          : "");
    }
  ...
  if (isPauschal && deal.applies_to) {
    ...
    const applyMap = {
      drinks: shopT("getraenke") || "Getränke",
      food: shopT("essen") || "Essen",
      all: shopT("alles") || "Alles",
    };
    applies.textContent =
      "✅ Gilt für: " + (applyMap[deal.applies_to] || deal.applies_to);
  ...
  if (deal.time_slots && deal.time_slots.length > 0) {
    ...
    var tsLabels = {
      morning: "\u{1F305} Morgen",
      midday: "\u2600\uFE0F Mittag",
      evening: "\u{1F319} Abend",
    };
    deal.time_slots.forEach(function (slot) {
      var sp = document.createElement("span");
      sp.style.cssText =
        "background:#2a2a2a;padding:3px 8px;border-radius:8px;font-size:11px;color:#ccc";
  ...
  favBtn.textContent =
    _favorites.indexOf(deal.id) !== -1 ? "\u2764\uFE0F" : "\u{1F90D}";
  ...
  if (deal.bar_address)
    addrDiv.textContent =
      "📍 " + deal.bar_address + (deal.bar_city ? ", " + deal.bar_city : "");
  ...
  if (isPauschal) {
    ...
    infoLine.textContent =
      deal.discount_percent + "% ab " + deal.min_order + " CHF";
...
async function doBuy() {
  ...
  if (!name || !email) {
    showToast(
      shopT("nameEmailRequired") || "Name und Email sind Pflichtfelder",
      true,
    );
    return;
  }
  if (!consent) {
    showToast(
      shopT("buyConsentError") || "Bitte AGB & Datenschutz akzeptieren",
      true,
    );
    return;
  }
...
function renderOrders(orders) {
  ...
  if (!orders.length) {
    ...
    div.innerHTML =
      "<h3>" +
      (shopT("keineBestellungen") || "Keine Bestellungen") +
      "</h3><p>" +
      (shopT("nochNichts") || "Du hast noch nichts bestellt") +
      "</p>";
    el.appendChild(div);
    return;
  }
  ...
  if (o.voucher_code) {
    ...
    codes.forEach(function (vc) {
      ...
      viewLink.style.cssText =
        "padding:7px 14px;border-radius:18px;background:#FF3366;color:#fff;font-size:12px;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:4px";
        ...
        nsBtn.addEventListener(
          ...
          (function (c, t) {
            return function () {
              ...
              navigator
                .share({ title: t || "BarSclusive Voucher", url: vUrl })
                .catch(function () {});
            };
          })(vc, o.deal_title || ""),
        );
...
async function doRefund(orderId) {
  ...
  const s = sessionGet();
  if (!s) {
    showToast(shopT("notLoggedIn") || "Nicht eingeloggt", true);
    return;
  }
  ...
  try {
    ...
    if (r.success) {
      showToast("✅ Rückerstattung angefordert");
      loadOrders();
...
async function doLogin() {
  ...
  try {
    ...
    if (_loginBtn) {
      ...
      _loginBtn.textContent = defaultLabel;
    }
  }
}
...
async function doRegister() {
  ...
  var passConfirm = document.getElementById("regPasswordConfirm")
  ...
  if (!consent) {
    showToast("Datenschutz akzeptieren", true);
    return;
  }
...
async function doLogout() {
  ...
  if (s) {
    try {
      await api({ action: "logout", token: s.token });
    } catch (e) {}
  }
  sessionClear();
  showView("deals");
...
function showView(view) {
  document.getElementById("dealsView").style.display =
    view === "deals" ? "block" : "none";
  document.getElementById("ordersView").style.display =
    view === "orders" ? "block" : "none";
  var fv = document.getElementById("favoritesView");
  if (fv) fv.style.display = view === "favorites" ? "block" : "none";
  document
    .getElementById("btnDeals")
    .classList.toggle("active", view === "deals");
  document
    .getElementById("btnOrders")
    .classList.toggle("active", view === "orders");
  var bf3 = document.getElementById("btnFavorites");
  if (bf3) bf3.classList.toggle("active", view === "favorites");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".filter-btn[data-filter-date]").forEach((btn) => {
    btn.addEventListener("click", function () {
      setFilter("date", this.dataset.filterDate, this);
    });
  });
  ...
  document.querySelectorAll(".filter-btn[data-filter-time]").forEach((btn) => {
    btn.addEventListener("click", function () {
      setFilter("time", this.dataset.filterTime, this);
    });
  });
  ...
  document.querySelectorAll(".filter-btn[data-filter-cat]").forEach((btn) => {
    btn.addEventListener("click", function () {
      setFilter("cat", this.dataset.filterCat, this);
    });
  });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (linkForgotPassword)
    linkForgotPassword.addEventListener("click", openResetPasswordModal);

  const btnSendResetCode = document.getElementById("btnSendResetCode");
  if (btnSendResetCode)
    btnSendResetCode.addEventListener("click", sendResetCode);

  const btnResetPassword = document.getElementById("btnResetPassword");
  if (btnResetPassword)
    btnResetPassword.addEventListener("click", resetPasswordSubmit);

  const btnBackToStep1 = document.getElementById("btnBackToStep1");
  if (btnBackToStep1)
    btnBackToStep1.addEventListener("click", backToResetStep1);
...
function shopT(key) {
  return (
    (SHOP_TRANSLATIONS[_shopLang] || {})[key] ||
    (SHOP_TRANSLATIONS.de || {})[key] ||
    ""
  );
}
const SHOP_TRANSLATIONS = {
  de: {
    ...
    abend: "Abend",
    suchen: "Suchen",
    profitiere: "Profitiere jetzt!",
    bestellt: "Bestellt:",
    gutscheinCode: "Gutschein-Code:",
    keineDeals: "Keine Deals gefunden",
    andereFilter: "Andere Filter versuchen",
    keineBestellungen: "Keine Bestellungen",
    nochNichts: "Du hast noch nichts bestellt",
    forgotPw: "Passwort vergessen?",
    sendCode: "Code senden",
    abbrechen: "Abbrechen",
    code6: "Code (6-stellig)",
    neuesPw: "Neues Passwort (mind. 8 Zeichen)",
    pwAendern: "Passwort ändern",
    nurAm: "Nur am",
    giltFuer: "Gilt für:",
    getraenke: "Getränke",
    essen: "Essen",
    alles: "Alles",
    refundReq: "Rückerstattung anfordern",
    ...
    fUeberUns: "Über uns",
    fSoFunktionierts: "So funktionierts",
    fImpressum: "Impressum",
    fDatenschutz: "Datenschutz",
    fAGB: "AGB",
    fKontakt: "Kontakt",
    fFuerBars: "Für Bars → Bar-Portal",
    loginSubmitBtn: "Einloggen",
    cancelLoginBtn: "Abbrechen",
    suchenBtn: "Suchen",
    anmeldenTitle: "Anmelden",
    searchBarDeal: "Bar oder Deal suchen...",
    searchPLZ: "PLZ oder Ort...",
    gutscheinAnzeigen: "Anzeigen",
    linkKopieren: "Link kopieren",
    linkKopiert: "Link kopiert!",
    warenkorbLeer: "Warenkorb ist leer",
    jetztBezahlen: "Jetzt bezahlen",
    total: "Total",
    deinName: "Dein Name:",
    deineEmail: "Deine Email:",
    pwBestaetigen: "Passwort bestätigen",
    viewDeals: "🏠 Deals",
    viewMap: "🗺️ Karte",
    catBreakfast: "🥐 Breakfast",
    catLunch: "🍽️ Lunch",
    catAperitif: "🍹 Aperitif",
    catDinner: "🍷 Dinner",
    catEvents: "🎉 Events",
    catDiscount: "🏷️ Rabatt",
    myLocationBtn: "Mein Standort",
    clearLocationBtn: "Zurücksetzen",
    geoEnableBtn: "Standort freigeben",
    geoBannerText: "Deals in deiner Nähe anzeigen?",
    mapStateText: "Deals direkt auf der Karte der Schweiz",
    mapCountryBadge: "🇨🇭 Schweiz",
    distanceAway: "entfernt",
    emailLbl: "Email",
    passwordLbl: "Passwort",
    nameLbl: "Name",
    registerPasswordLbl: "Passwort (mind. 8 Zeichen)",
    registerConfirmLbl: "Passwort bestätigen",
    registerModalTitle: "✨ Registrieren",
    registerSubmitBtn: "Registrieren",
    acceptPrivacyOnly: "Ich akzeptiere die",
    privacyOnly: "Datenschutzerklärung",
    alreadyRegistered: "Schon registriert?",
    goToLogin: "Zum Login",
    cartTitle: "🛒 Warenkorb",
    checkoutTitle: "Checkout",
    acceptTermsCart: "Ich akzeptiere die",
    processing: "⏳ Wird verarbeitet...",
    nameEmailRequired: "Name und Email sind Pflichtfelder",
    checkoutFailed: "Checkout fehlgeschlagen",
    loginModalTitle: "🔐 Anmelden",
    resetModalTitle: "🔑 Passwort zurücksetzen",
    resetInfo1:
      "Gib deine Email-Adresse ein. Wir senden dir einen 6-stelligen Code.",
    resetInfo2:
      "Gib den 6-stelligen Code aus deiner Email ein und wähle ein neues Passwort.",
    backBtn: "Zurück",
  },
  en: {
    deals: "Deals",
    ...
    abend: "Evening",
    suchen: "Search",
    profitiere: "Get this deal!",
    bestellt: "Ordered:",
    gutscheinCode: "Voucher code:",
    keineDeals: "No deals found",
    andereFilter: "Try different filters",
    keineBestellungen: "No orders",
    nochNichts: "You haven't ordered anything yet",
    forgotPw: "Forgot password?",
    sendCode: "Send code",
    abbrechen: "Cancel",
    code6: "Code (6 digits)",
    neuesPw: "New password (min. 8 characters)",
    pwAendern: "Change password",
    nurAm: "Only on",
    giltFuer: "Applies to:",
    getraenke: "Drinks",
    essen: "Food",
    alles: "Everything",
    refundReq: "Request refund",
    ...
    fUeberUns: "About Us",
    fSoFunktionierts: "How It Works",
    fImpressum: "Legal Notice",
    fDatenschutz: "Privacy",
    fAGB: "Terms",
    fKontakt: "Contact",
    fFuerBars: "For Bars → Bar Portal",
    loginSubmitBtn: "Login",
    cancelLoginBtn: "Cancel",
    suchenBtn: "Search",
    anmeldenTitle: "Login",
    searchBarDeal: "Search bar or deal...",
    searchPLZ: "ZIP or city...",
    gutscheinAnzeigen: "View",
    linkKopieren: "Copy link",
    linkKopiert: "Link copied!",
    warenkorbLeer: "Cart is empty",
    jetztBezahlen: "Pay now",
    total: "Total",
    deinName: "Your name:",
    deineEmail: "Your email:",
    pwBestaetigen: "Confirm password",
    viewDeals: "🏠 Deals",
    viewMap: "🗺️ Map",
    catBreakfast: "🥐 Breakfast",
    catLunch: "🍽️ Lunch",
    ...
    catDinner: "🍷 Dinner",
    catEvents: "🎉 Events",
    catDiscount: "🏷️ Discount",
    myLocationBtn: "My location",
    clearLocationBtn: "Reset",
    geoEnableBtn: "Enable location",
    geoBannerText: "Show deals near you?",
    mapStateText: "Deals directly on the map of Switzerland",
    mapCountryBadge: "🇨🇭 Switzerland",
    distanceAway: "away",
    emailLbl: "Email",
    passwordLbl: "Password",
    nameLbl: "Name",
    registerPasswordLbl: "Password (min. 8 chars)",
    registerConfirmLbl: "Confirm password",
    registerModalTitle: "✨ Register",
    registerSubmitBtn: "Register",
    acceptPrivacyOnly: "I accept the",
    privacyOnly: "Privacy Policy",
    alreadyRegistered: "Already registered?",
    goToLogin: "Go to login",
    cartTitle: "🛒 Cart",
    checkoutTitle: "Checkout",
    acceptTermsCart: "I accept the",
    processing: "⏳ Processing...",
    nameEmailRequired: "Name and email are required",
    checkoutFailed: "Checkout failed",
    loginModalTitle: "🔐 Login",
    resetModalTitle: "🔑 Reset password",
    resetInfo1: "Enter your email address. We will send you a 6-digit code.",
    resetInfo2:
  ...
  it: {
    ...
    abend: "Sera",
    suchen: "Cerca",
    profitiere: "Approfitta ora!",
    bestellt: "Ordinato:",
    gutscheinCode: "Codice voucher:",
    keineDeals: "Nessun deal trovato",
    andereFilter: "Prova altri filtri",
    keineBestellungen: "Nessun ordine",
    nochNichts: "Non hai ancora ordinato nulla",
    forgotPw: "Password dimenticata?",
    sendCode: "Invia codice",
    abbrechen: "Annulla",
    code6: "Codice (6 cifre)",
    neuesPw: "Nuova password (min. 8 caratteri)",
    pwAendern: "Cambia password",
    nurAm: "Solo il",
    giltFuer: "Valido per:",
    getraenke: "Bevande",
    essen: "Cibo",
    alles: "Tutto",
    refundReq: "Richiedi rimborso",
    ...
    fUeberUns: "Chi siamo",
    fSoFunktionierts: "Come funziona",
    fImpressum: "Impressum",
    fDatenschutz: "Privacy",
    fAGB: "Condizioni",
    fKontakt: "Contatto",
    fFuerBars: "Per bar → Portale Bar",
    loginSubmitBtn: "Accedi",
    cancelLoginBtn: "Annulla",
    suchenBtn: "Cerca",
    anmeldenTitle: "Accedi",
    searchBarDeal: "Cerca bar o offerta...",
    searchPLZ: "CAP o città...",
    gutscheinAnzeigen: "Visualizza",
    linkKopieren: "Copia link",
    linkKopiert: "Link copiato!",
    warenkorbLeer: "Carrello vuoto",
    jetztBezahlen: "Paga ora",
    total: "Totale",
    deinName: "Il tuo nome:",
    deineEmail: "La tua email:",
    pwBestaetigen: "Conferma password",
    viewDeals: "🏠 Deals",
    viewMap: "🗺️ Mappa",
    catBreakfast: "🥐 Colazione",
    catLunch: "🍽️ Pranzo",
    catAperitif: "🍹 Aperitivo",
    catDinner: "🍷 Cena",
    catEvents: "🎉 Eventi",
    catDiscount: "🏷️ Sconto",
    myLocationBtn: "La mia posizione",
    clearLocationBtn: "Reimposta",
    geoEnableBtn: "Attiva posizione",
    geoBannerText: "Mostrare i deal vicino a te?",
    mapStateText: "Deal direttamente sulla mappa della Svizzera",
    mapCountryBadge: "🇨🇭 Svizzera",
    distanceAway: "di distanza",
    emailLbl: "Email",
    passwordLbl: "Password",
    nameLbl: "Nome",
    registerPasswordLbl: "Password (min. 8 caratteri)",
    registerConfirmLbl: "Conferma password",
    registerModalTitle: "✨ Registrati",
    registerSubmitBtn: "Registrati",
    acceptPrivacyOnly: "Accetto la",
    privacyOnly: "Privacy",
    alreadyRegistered: "Già registrato?",
    goToLogin: "Vai al login",
    cartTitle: "🛒 Carrello",
    checkoutTitle: "Checkout",
    acceptTermsCart: "Accetto",
    processing: "⏳ Elaborazione...",
    nameEmailRequired: "Nome ed email sono obbligatori",
    checkoutFailed: "Checkout non riuscito",
    loginModalTitle: "🔐 Accedi",
    resetModalTitle: "🔑 Reimposta password",
    resetInfo1: "Inserisci la tua email. Ti invieremo un codice di 6 cifre.",
    resetInfo2:
      "Inserisci il codice di 6 cifre ricevuto via email e scegli una nuova password.",
    backBtn: "Indietro",
  },
  fr: {
    deals: "Deals",
    ...
    heroSub:
      "Offres exclusives pour petit-déjeuner, brunch, apéritif et événements",
    buyBtn: "Acheter Deal",
    ...
    profitiere: "Profite maintenant!",
    bestellt: "Commandé:",
    gutscheinCode: "Code bon:",
    keineDeals: "Aucune offre trouvée",
    andereFilter: "Essaie d'autres filtres",
    keineBestellungen: "Aucune commande",
    nochNichts: "Tu n'as encore rien commandé",
    forgotPw: "Mot de passe oublié?",
    sendCode: "Envoyer le code",
    abbrechen: "Annuler",
    code6: "Code (6 chiffres)",
    neuesPw: "Nouveau mot de passe (min. 8 caractères)",
    pwAendern: "Changer",
    nurAm: "Uniquement le",
    giltFuer: "Valable pour:",
    getraenke: "Boissons",
    essen: "Nourriture",
    alles: "Tout",
    refundReq: "Demander remboursement",
    ...
    fUeberUns: "À propos",
    fSoFunktionierts: "Comment ça marche",
    fImpressum: "Mentions légales",
    fDatenschutz: "Confidentialité",
    fAGB: "CGV",
    fKontakt: "Contact",
    fFuerBars: "Pour bars → Portail Bar",
    loginSubmitBtn: "Connexion",
    cancelLoginBtn: "Annuler",
    suchenBtn: "Chercher",
    anmeldenTitle: "Connexion",
    searchBarDeal: "Chercher bar ou offre...",
    searchPLZ: "NPA ou ville...",
    gutscheinAnzeigen: "Afficher",
    ...
    catLunch: "🍽️ Déjeuner",
    catAperitif: "🍹 Apéritif",
    catDinner: "🍷 Dîner",
    catEvents: "🎉 Événements",
    catDiscount: "🏷️ Réduction",
    myLocationBtn: "Ma position",
    clearLocationBtn: "Réinitialiser",
    geoEnableBtn: "Activer la position",
    geoBannerText: "Afficher les deals près de vous ?",
    mapStateText: "Deals directement sur la carte de la Suisse",
    mapCountryBadge: "🇨🇭 Suisse",
    distanceAway: "de distance",
    emailLbl: "Email",
    passwordLbl: "Mot de passe",
    nameLbl: "Nom",
    registerPasswordLbl: "Mot de passe (min. 8 car.)",
    registerConfirmLbl: "Confirmer le mot de passe",
    registerModalTitle: "✨ Inscription",
    registerSubmitBtn: "S'inscrire",
    acceptPrivacyOnly: "J'accepte la",
    privacyOnly: "Confidentialité",
    alreadyRegistered: "Déjà inscrit ?",
    goToLogin: "Vers la connexion",
    cartTitle: "🛒 Panier",
    checkoutTitle: "Checkout",
    acceptTermsCart: "J'accepte les",
    processing: "⏳ Traitement...",
    nameEmailRequired: "Nom et email sont obligatoires",
    checkoutFailed: "Échec du checkout",
    loginModalTitle: "🔐 Connexion",
    resetModalTitle: "🔑 Réinitialiser le mot de passe",
    resetInfo1:
      "Saisissez votre adresse email. Nous vous enverrons un code à 6 chiffres.",
    resetInfo2:
      "Saisissez le code à 6 chiffres reçu par email puis choisissez un nouveau mot de passe.",
    backBtn: "Retour",
  },
};
...
let shopLang = "de";
function st(key) {
  return SHOP_TRANSLATIONS[shopLang][key] || SHOP_TRANSLATIONS.de[key] || key;
}
...
function setShopLang(lang) {
  ...
  document.querySelectorAll(".shop-lang-btn").forEach(function (b) {
...
function applyShopTranslations() {
  ...
  var rm = document.getElementById("registerModalTitle");
  if (rm) rm.textContent = st("registerModalTitle");
  var ub = document.getElementById("userBtn");
  if (ub && !sessionGet()) ub.textContent = "👤 " + st("loginBtn");
  var ct = document.getElementById("cartTitle");
  if (ct) ct.textContent = st("cartTitle");
...
async function doChangePassword() {
  ...
  if (!oldPw || !newPw || !confPw) {
    err.textContent = shopT("allFieldsRequired") || "Alle Felder ausfüllen.";
    return;
  }
  if (newPw.length < 8) {
    err.textContent =
      shopT("newPasswordMinErr") || "Neues Passwort mind. 8 Zeichen.";
    return;
  }
  if (newPw !== confPw) {
    err.textContent =
      shopT("passwordMismatchDot") || "Passwörter stimmen nicht überein.";
    return;
  }
  try {
    const r = await api({
    ...
    if (r.success) {
      ...
      err.textContent = translateShopRuntimeMessage(
        r.error || shopT("genericErrorDot") || "Fehler.",
      );
    }
  } catch (e) {
    err.textContent = shopT("networkErrorDot") || "Verbindungsfehler.";
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (btnUseMyLocation)
    btnUseMyLocation.addEventListener("click", requestGeoPermission);
  ...
  if (locInput) {
        ...
        timer = setTimeout(async function () {
          ...
          results.forEach(function (item) {
            ...
            row.innerHTML =
              ...
              escHtml(p.label) +
              '</div><div class="suggestion-meta">' +
              escHtml(p.shortLabel) +
              "</div>";
    ...
    locInput.addEventListener("focus", function () {
      if (locSuggestions && locSuggestions.innerHTML.trim())
        locSuggestions.style.display = "block";
    });
    document.addEventListener("click", function (e) {
      if (!e.target.closest(".search-select-wrap")) closeLocationSuggestions();
    });
  }
  updateLocationUi();
});
...
async function toggleFavorite(dealId, btn) {
  var s = sessionGet();
  if (!s) {
    showToast(shopT("loginRequired") || "Bitte einloggen", true);
    return;
  }
  ...
  try {
    ...
    if (r.success) {
      if (isFav)
        _favorites = _favorites.filter(function (id) {
          return id !== dealId;
        });
...
function showFavorites() {
  ...
  if (!_favorites.length) {
    el.innerHTML =
      '<div class="empty"><h3>Noch keine Favoriten</h3><p>Klicke \u2764\uFE0F bei einem Deal</p></div>';
    return;
  }
  var favDeals = allDeals.filter(function (d) {
    return _favorites.indexOf(d.id) !== -1;
  });
  if (!favDeals.length) {
    el.innerHTML =
      '<div class="empty"><h3>Keine aktiven Deals von Favoriten</h3></div>';
    return;
  }
  favDeals.forEach(function (d) {
    el.appendChild(buildDealCard(d));
  });
}
...
function openDealDetail(deal) {
  ...
  if (deal.image_url) {
    ...
    if (u.indexOf("lh3.googleusercontent.com/d/") >= 0)
      gid = u.split("/d/")[1].split("=")[0];
    else if (u.indexOf("thumbnail?id=") >= 0)
      gid = u.split("id=")[1].split("&")[0];
  ...
  document.getElementById("ddPrice").textContent =
    Number(deal.deal_price).toFixed(2) + " CHF";
  ...
  if (deal.bar_address)
    info +=
      ...
      "</span><span>" +
      escHtml(deal.bar_address) +
      ", " +
      escHtml(deal.bar_zip || "") +
      " " +
      escHtml(deal.bar_city || "") +
      "</span></div>";
  if (isPauschal) {
    if (deal.discount_percent)
      info +=
        '<div><span style="color:#999">Rabatt</span><span style="color:#FF3366;font-weight:700">' +
        deal.discount_percent +
        "%</span></div>";
    if (deal.min_order)
      info +=
        '<div><span style="color:#999">Mindestbestellung</span><span>' +
        deal.min_order +
        " CHF</span></div>";
    info +=
      '<div><span style="color:#999">Typ</span><span>Pauschalgutschein</span></div>';
  }
  var weekdays = deal.valid_weekdays || [];
  if (weekdays.length)
    info +=
      '<div><span style="color:#999">Wochentage</span><span>' +
      weekdays.join(", ") +
      "</span></div>";
  if (deal.valid_from_time && deal.valid_to_time)
    info +=
      '<div><span style="color:#999">Zeit</span><span>' +
      deal.valid_from_time +
      " - " +
      deal.valid_to_time +
      "</span></div>";
  if (deal.max_quantity > 0)
    info +=
      '<div><span style="color:#999">Verfügbar</span><span>' +
      Math.max(0, deal.max_quantity - (deal.sold_count || 0)) +
      " / " +
      deal.max_quantity +
      "</span></div>";
  document.getElementById("ddInfo").innerHTML =
    info || '<div style="color:#666">Keine weiteren Details</div>';

  // Share buttons
  var shareUrl =
    window.location.origin + window.location.pathname + "?deal=" + deal.id;
  var shareText =
    deal.title +
    " bei " +
    deal.bar_name +
  ...
  if (navigator.share) {
    shareItems.unshift([
      ...
      function () {
        navigator
          .share({ title: deal.title, text: shareText, url: shareUrl })
          .catch(function () {});
      },
    ]);
  }
  ...
  // Buy button
  document.getElementById("ddBuyBtn").onclick = function () {
    closeDealDetail();
    openBuyModal(deal);
  };
  ...
  if (!ddCartBtn) {
    ...
    ddCartBtn.style.cssText =
      "width:100%;background:#2a2a2a;border:1px solid #3a3a3a;color:#fff;padding:12px;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;margin-top:8px";
    document.getElementById("ddBuyBtn").after(ddCartBtn);
  }
  ddCartBtn.textContent = "🛒 " + (shopT("cartAddTitle") || "In den Warenkorb");
  ddCartBtn.onclick = function () {
    addToCart(deal);
    closeDealDetail();
  };
...
function shareDeal(platform) {
  ...
  var url = encodeURIComponent(
    window.location.origin + window.location.pathname + "?deal=" + d.id,
  );
...
function shareVoucher(code, dealTitle, platform) {
  ...
  var text = encodeURIComponent(
    (dealTitle || "BarSclusive Gutschein") + " - " + code,
  );
  var link = "";
  if (platform === "whatsapp")
    link = "https://wa.me/?text=" + text + "%20" + url;
...
window.addEventListener("load", function () {
  ...
  if (dealId && allDeals.length) {
    var d = allDeals.find(function (x) {
      return x.id === dealId;
    });
    if (d)
      setTimeout(function () {
...
function requestGeoPermission() {
  ...
  navigator.geolocation.getCurrentPosition(
    function (pos) {
      ...
      _locationState = {
        label: shopT("myLocationBtn") || "Mein Standort",
        lat: _userLat,
        lng: _userLng,
        source: "geo",
        textFilter: "",
      ...
      dismissGeoBanner();
      showToast(
        shopT("dealsSortedByDistance") || "📍 Deals werden nach Nähe sortiert",
      );
      sortDealsByDistance();
    },
    function () {
      dismissGeoBanner();
      showToast(
        shopT("locationUnavailable") || "Standort nicht verfügbar",
        true,
      );
    },
...
function getCart() {
  try {
    _cart = JSON.parse(localStorage.getItem("barsclusive_cart") || "[]");
  } catch (e) {
    _cart = [];
  }
  return _cart;
...
function saveCart() {
  try {
    localStorage.setItem("barsclusive_cart", JSON.stringify(_cart));
  } catch (e) {}
  updateCartBadge();
}
...
function addToCart(deal) {
  ...
  showToast(
    "🛒 " +
      deal.title +
      " " +
      (shopT("addedToCartSuffix") || "zum Warenkorb hinzugefügt"),
  );
}
...
function changeCartQty(dealId, delta) {
  getCart();
  var item = _cart.find(function (c) {
    return c.deal_id === dealId;
  });
  if (!item) return;
...
function updateCartBadge() {
  ...
  if (openBtn) {
    ...
    openBtn.setAttribute(
      "aria-label",
      (shopT("cartHeaderBtn") || "Warenkorb") +
        (count > 0 ? " (" + count + ")" : ""),
    );
    openBtn.title =
      count > 0
        ? (shopT("cartHeaderBtn") || "Warenkorb") + " (" + count + ")"
        : shopT("cartHeaderBtn") || "Warenkorb";
  }
}
...
function renderCartPanel() {
  ...
  if (!_cart.length) {
    body.innerHTML =
      ...
      (shopT("warenkorbLeer") || "Warenkorb ist leer") +
      "</div>";
    return;
  }
  var s = sessionGet();
  ...
  _cart.forEach(function (c) {
    var row = document.createElement("div");
    row.style.cssText =
      "display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid #222;gap:12px";
    ...
    btnMinus.style.cssText =
      "background:#333;color:#fff;border:none;width:28px;height:28px;border-radius:50%;cursor:pointer;font-size:16px";
    btnMinus.addEventListener("click", function () {
      changeCartQty(c.deal_id, -1);
    });
  ...
  totalDiv.style.cssText =
    "padding:16px 0;font-size:18px;font-weight:700;text-align:right;border-top:2px solid #FF3366;margin-top:8px";
  totalDiv.textContent =
    (shopT("total") || "Total") + ": " + getCartTotal().toFixed(2) + " CHF";
  body.appendChild(totalDiv);

  var buyerWrap = document.createElement("div");
  buyerWrap.style.cssText =
    "margin-top:14px;padding:14px;background:#171717;border:1px solid #2a2a2a;border-radius:12px";
  buyerWrap.innerHTML =
    "" +
    '<div style="font-size:13px;font-weight:700;margin-bottom:12px">' +
    (shopT("checkoutTitle") || "Checkout") +
    "</div>" +
    '<div class="form-group" style="margin-bottom:10px"><label class="form-label" for="cartBuyerName">' +
    (shopT("nameLbl") || "Name") +
    '</label><input type="text" class="form-input" id="cartBuyerName" value="' +
    escHtml(defaultName) +
    '" autocomplete="name"></div>' +
    '<div class="form-group" style="margin-bottom:10px"><label class="form-label" for="cartBuyerEmail">' +
    (shopT("emailLbl") || "Email") +
    '</label><input type="email" class="form-input" id="cartBuyerEmail" value="' +
    escHtml(defaultEmail) +
    '" autocomplete="email"></div>' +
    '<label style="display:flex;gap:8px;align-items:flex-start;font-size:13px;color:#ccc"><input type="checkbox" id="cartConsent"><span>' +
    (shopT("acceptTermsCart") || "Ich akzeptiere die") +
    ' <a href="agb.html" target="_blank" rel="noopener" style="color:#FF3366">' +
    (shopT("fAGB") || "AGB") +
    "</a> " +
    (shopLang === "en"
      ? "and"
      : shopLang === "it"
        ...
        : shopLang === "fr"
          ? "et"
          : "und") +
    ' <a href="datenschutz.html" target="_blank" rel="noopener" style="color:#FF3366">' +
    (shopT("fDatenschutz") || "Datenschutz") +
    "</a>.</span></label>";
  body.appendChild(buyerWrap);

  var checkBtn = document.createElement("button");
  checkBtn.id = "cartCheckoutBtn";
  checkBtn.textContent = _cartCheckoutBusy
    ? shopT("processing") || "⏳ Wird verarbeitet..."
    : shopT("jetztBezahlen") || "Jetzt bezahlen";
  checkBtn.disabled = _cartCheckoutBusy;
  checkBtn.style.cssText =
    "width:100%;background:#FF3366;color:#fff;border:none;padding:14px;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer;margin-top:12px";
...
async function checkoutCart() {
  getCart();
  if (!_cart.length || _cartCheckoutBusy) {
    if (!_cart.length)
      showToast(shopT("warenkorbLeer") || "Warenkorb ist leer", true);
    return;
  }
  var s = sessionGet();
    ...
    showToast(
      shopLang === "en"
        ...
        : shopLang === "it"
          ...
          : shopLang === "fr"
            ? "Veuillez accepter les CGV et la confidentialité"
  ...
  if (checkoutBtn) {
    ...
    checkoutBtn.textContent = shopT("processing") || "⏳ Wird verarbeitet...";
  }
  ...
  try {
    var r = await api({
      ...
      items: _cart.map(function (c) {
        return { deal_id: c.deal_id, quantity: c.quantity };
      }),
      buyer_name: buyerName,
      buyer_email: buyerEmail,
      customer_id: s ? s.customerId || s.token || "" : "",
      site_url:
        window.location.origin +
        window.location.pathname.replace(/\/[^\/]*$/, ""),
    });
    ...
    showToast(
      r.error || shopT("checkoutFailed") || "Checkout fehlgeschlagen",
      true,
    );
  } catch (e) {
    showToast("Verbindungsfehler", true);
...
var _shopMapMarkers = [];
var _mapView = false;
var _swissBounds = L.latLngBounds([
  [45.75, 5.85],
  [47.92, 10.75],
]);
...
function buildMapPopupHtml(barEntry) {
  var html =
    ...
    escHtml(barEntry.name || "") +
    '</div><div class="map-popup-sub">' +
    escHtml(barEntry.city || "") +
    '</div><div class="map-popup-list">';
  barEntry.deals.slice(0, 6).forEach(function (d) {
    html +=
      '<div class="map-popup-item" data-map-deal-id="' +
      d.id +
      '"><span>' +
      escHtml(d.title || "") +
      '</span><span class="map-popup-price">' +
      Number(d.deal_price || 0).toFixed(2) +
      " CHF</span></div>";
  });
  if (barEntry.deals.length > 6) {
    html +=
      '<div style="font-size:12px;color:#666;padding-top:2px">+' +
      (barEntry.deals.length - 6) +
      " weitere Deals</div>";
  }
  html += "</div></div>";
  return html;
...
function buildPricePinIcon(barEntry) {
  ...
  var count =
    barEntry.deals.length > 1
      ? "<small>+" + (barEntry.deals.length - 1) + "</small>"
      : "";
  return L.divIcon({
    className: "map-pin-wrapper",
    html:
      '<div class="map-price-pin"><span>' +
      label +
      "</span>" +
      count +
      "</div>",
...
function offsetDuplicateDealsForMap(deals) {
  ...
  Object.keys(groups).forEach(function (key) {
    ...
    var baseLat = Number(arr[0].bar_lat),
      baseLng = Number(arr[0].bar_lng);
    var radius = 0.00022;
...
function updateShopMapMarkers() {
  if (!_shopMap) return;
  _shopMapMarkers.forEach(function (m) {
    try {
      _shopMap.removeLayer(m);
    } catch (e) {}
  });
  _shopMapMarkers = [];
  ...
  if (
    isValidCoord(Number(_userLat)) &&
    isValidCoord(Number(_userLng)) &&
    _swissBounds.contains([Number(_userLat), Number(_userLng)])
  ) {
    var userMarker = L.marker([Number(_userLat), Number(_userLng)], {
      icon: L.divIcon({
        className: "map-user-wrapper",
  ...
  visibleDeals.forEach(function (d) {
    var lat = Number(d._mapLat != null ? d._mapLat : d.bar_lat),
      lng = Number(d._mapLng != null ? d._mapLng : d.bar_lng);
    if (!isValidCoord(lat) || !isValidCoord(lng)) return;
    if (!_swissBounds.contains([lat, lng])) return;
    var marker = L.marker([lat, lng], {
      icon: buildDealPinIcon(d),
      keyboard: true,
      title: d.title || "",
    }).addTo(_shopMap);
  ...
  if (mapStateText) {
    ...
    mapStateText.textContent = visibleDeals.length
      ? visibleDeals.length + " " + st("deals") + " · " + countryLabel
      : st("mapStateText");
  }

  if (bounds.length > 1) {
    _shopMap.fitBounds(L.latLngBounds(bounds).pad(0.18), {
      padding: [20, 20],
      maxZoom: 14,
    });
...
function toggleMapView(showMap) {
  ...
  if (showMap) {
    ...
    if (btnMap) {
      btnMap.style.background = "#FF3366";
      btnMap.style.color = "#fff";
      btnMap.style.borderColor = "#FF3366";
    }
    if (btnGrid) {
      btnGrid.style.background = "#2a2a2a";
      btnGrid.style.color = "#ccc";
      btnGrid.style.borderColor = "#3a3a3a";
    }
    initShopMap();
    setTimeout(function () {
      if (_shopMap) _shopMap.invalidateSize();
      updateShopMapMarkers();
    }, 120);
  } else {
    if (mapWrap) mapWrap.style.display = "none";
    if (dealsView) dealsView.style.display = "block";
    if (btnGrid) {
      btnGrid.style.background = "#FF3366";
      btnGrid.style.color = "#fff";
      btnGrid.style.borderColor = "#FF3366";
    }
    if (btnMap) {
      btnMap.style.background = "#2a2a2a";
      btnMap.style.color = "#ccc";
      btnMap.style.borderColor = "#3a3a3a";
    }
  }
}
...
(function () {
  ...
  setShopLang = function (lang) {
    ...
    var t = shopT;
    var el;
    el = document.getElementById("registerModalTitle");
    if (el) el.textContent = t("registerModalTitle");
    el = document.querySelector('label[for="regName"]');
    if (el) el.textContent = t("nameLbl");
    el = document.querySelector('label[for="regEmail"]');
    if (el) el.textContent = t("emailLbl");
    el = document.querySelector('label[for="regPassword"]');
    if (el) el.textContent = t("registerPasswordLbl");
    el = document.querySelector('label[for="regPasswordConfirm"]');
    if (el) el.textContent = t("registerConfirmLbl");
  };
})();
...
Object.assign(SHOP_TRANSLATIONS.de, {
  ...
  networkReload: "Verbindungsfehler - bitte neu laden",
  dealsLoadError: "Fehler beim Laden der Deals",
  cartAddTitle: "In den Warenkorb",
  acceptTermsPrivacy: "Bitte AGB & Datenschutz akzeptieren",
  invalidCredentials: "Ungültige Zugangsdaten",
  fillAllFields: "Alle Felder ausfüllen",
  passwordMinErr: "Passwort mind. 8 Zeichen",
  passwordMismatch: "Passwörter stimmen nicht überein",
  emailRequired: "Bitte Email eingeben",
  ...
  refundRequestedSuccess: "✅ Rückerstattung angefordert",
  locationUnavailable: "Standort nicht verfügbar",
  dealsSortedByDistance: "📍 Deals werden nach Nähe sortiert",
  addedToCartSuffix: "zum Warenkorb hinzugefügt",
  processing: "⏳ Wird verarbeitet...",
  loadError: "Fehler",
  registrationSuccess: "✅ Registrierung erfolgreich!",
  loginSuccess: "✅ Eingeloggt!",
  logoutSuccess: "Ausgeloggt",
  linkGenericPrefix: "Link:",
  nameEmailRequired: "Name und Email sind Pflichtfelder",
  buyConsentError: "Bitte AGB & Datenschutz akzeptieren",
  allFieldsRequired: "Alle Felder ausfüllen.",
  newPasswordMinErr: "Neues Passwort mind. 8 Zeichen.",
  passwordMismatchDot: "Passwörter stimmen nicht überein.",
  genericErrorDot: "Fehler.",
  networkErrorDot: "Verbindungsfehler.",
});
Object.assign(SHOP_TRANSLATIONS.en, {
  shareBtn: "Share",
  networkError: "Connection error",
  networkReload: "Connection error - please reload",
  dealsLoadError: "Error loading deals",
  cartAddTitle: "Add to cart",
  acceptTermsPrivacy: "Please accept terms and privacy",
  invalidCredentials: "Invalid credentials",
  fillAllFields: "Please fill in all fields",
  passwordMinErr: "Password must be at least 8 characters",
  ...
  refundConfirm: "Request refund?\nThe voucher will become invalid.",
  refundRequestedSuccess: "✅ Refund requested",
  locationUnavailable: "Location not available",
  dealsSortedByDistance: "📍 Deals are now sorted by distance",
  ...
  linkGenericPrefix: "Link:",
  buyConsentError: "Please accept terms and privacy",
  allFieldsRequired: "Please fill in all fields.",
  newPasswordMinErr: "New password must be at least 8 characters.",
  passwordMismatchDot: "Passwords do not match.",
  genericErrorDot: "Error.",
  networkErrorDot: "Connection error.",
});
Object.assign(SHOP_TRANSLATIONS.it, {
  shareBtn: "Condividi",
  networkError: "Errore di connessione",
  networkReload: "Errore di connessione - ricarica la pagina",
  dealsLoadError: "Errore durante il caricamento dei deal",
  cartAddTitle: "Aggiungi al carrello",
  acceptTermsPrivacy: "Accetta condizioni e privacy",
  invalidCredentials: "Credenziali non valide",
  fillAllFields: "Compila tutti i campi",
  passwordMinErr: "La password deve avere almeno 8 caratteri",
  passwordMismatch: "Le password non coincidono",
  emailRequired: "Inserisci l'email",
  codeSent: "Codice inviato!",
  passwordChangedSuccess: "✅ Password modificata!",
  loginRequired: "Accedi prima",
  notLoggedIn: "Non connesso",
  refundConfirm: "Richiedere il rimborso?\nIl voucher diventerà non valido.",
  refundRequestedSuccess: "✅ Rimborso richiesto",
  locationUnavailable: "Posizione non disponibile",
  dealsSortedByDistance: "📍 I deal sono ora ordinati per distanza",
  addedToCartSuffix: "aggiunto al carrello",
  loadError: "Errore",
  registrationSuccess: "✅ Registrazione riuscita!",
  loginSuccess: "✅ Accesso effettuato!",
  logoutSuccess: "Disconnesso",
  linkGenericPrefix: "Link:",
  buyConsentError: "Accetta condizioni e privacy",
  allFieldsRequired: "Compila tutti i campi.",
  newPasswordMinErr: "La nuova password deve avere almeno 8 caratteri.",
  passwordMismatchDot: "Le password non coincidono.",
  genericErrorDot: "Errore.",
  networkErrorDot: "Errore di connessione.",
});
Object.assign(SHOP_TRANSLATIONS.fr, {
  shareBtn: "Partager",
  networkError: "Erreur de connexion",
  networkReload: "Erreur de connexion - veuillez recharger",
  dealsLoadError: "Erreur lors du chargement des deals",
  cartAddTitle: "Ajouter au panier",
  acceptTermsPrivacy: "Veuillez accepter les CGV et la confidentialité",
  invalidCredentials: "Identifiants invalides",
  fillAllFields: "Veuillez remplir tous les champs",
  passwordMinErr: "Le mot de passe doit contenir au moins 8 caractères",
  ...
  refundConfirm: "Demander un remboursement ?\nLe bon deviendra invalide.",
  refundRequestedSuccess: "✅ Remboursement demandé",
  locationUnavailable: "Position non disponible",
  dealsSortedByDistance: "📍 Les deals sont maintenant triés par distance",
  addedToCartSuffix: "ajouté au panier",
  loadError: "Erreur",
  registrationSuccess: "✅ Inscription réussie !",
  loginSuccess: "✅ Connecté !",
  logoutSuccess: "Déconnecté",
  linkGenericPrefix: "Lien :",
  buyConsentError: "Veuillez accepter les CGV et la confidentialité",
  allFieldsRequired: "Veuillez remplir tous les champs.",
  newPasswordMinErr:
    "Le nouveau mot de passe doit contenir au moins 8 caractères.",
  passwordMismatchDot: "Les mots de passe ne correspondent pas.",
  genericErrorDot: "Erreur.",
  networkErrorDot: "Erreur de connexion.",
});
...
loadDeals = async function (forceRefresh) {
  ...
  var lang = (
    _shopLang ||
    shopLang ||
    localStorage.getItem("barsclusive_lang") ||
    "de"
  ).toLowerCase();
  ...
  if (!forceRefresh) {
    try {
      ...
      if (localRaw) {
        ...
        if (
          localParsed.data &&
          localParsed.lang === lang &&
          now - localParsed.timestamp < CACHE_DURATION
        ) {
  ...
  try {
    const r = await fetch(
      BACKEND_URL + "?action=getActiveDeals&lang=" + encodeURIComponent(lang),
    );
    if (!r.ok) throw new Error("Network error");
    const d = await r.json();
    if (d.success) {
      ...
      showToast(
        d.error || shopT("dealsLoadError") || "Fehler beim Laden der Deals",
        true,
      );
    }
  } catch (e) {
    showToast(
      shopT("networkReload") || "Verbindungsfehler - bitte neu laden",
      true,
    );
  }
};
...
try {
  ...
  if (_localRaw) {
    ...
    if (
      _localParsed.data &&
      _localParsed.lang === _initialLang &&
      Date.now() - _localParsed.timestamp < 30 * 60 * 1000
    ) {
...
(function () {
  Object.assign(SHOP_TRANSLATIONS.de, {
    ...
    langFrench: "Français",
    acceptTermsRegister: "Ich akzeptiere die",
    andConnector: "und",
    acceptPrivacyOnly: "Ich akzeptiere die",
    privacyOnly: "Datenschutzerklärung",
    ...
    minimumOrderLabel: "Mindestbestellung",
    typeLabel: "Typ",
    flatVoucherType: "Pauschalgutschein",
    detailsNone: "Keine weiteren Details",
    originalBarText: "Originaltext der Bar",
    linkCopied: "Link kopiert!",
    loginSuccess: "Eingeloggt!",
    registerSuccess: "Registrierung erfolgreich!",
    emailPasswordRequired: "Bitte Email und Passwort eingeben",
    invalidCredentials: "Ungültige Zugangsdaten",
  ...
  Object.assign(SHOP_TRANSLATIONS.en, {
    ...
    langItalian: "Italian",
    langFrench: "French",
    acceptTermsRegister: "I accept the",
    andConnector: "and",
    acceptPrivacyOnly: "I accept the",
    privacyOnly: "Privacy Policy",
    acceptTermsPrivacy: "Please accept terms and privacy",
    ...
    typeLabel: "Type",
    flatVoucherType: "Flat discount voucher",
    detailsNone: "No further details",
    originalBarText: "Original bar text",
  ...
  Object.assign(SHOP_TRANSLATIONS.it, {
    ...
    langItalian: "Italiano",
    langFrench: "Francese",
    acceptTermsRegister: "Accetto i",
    andConnector: "e",
    acceptPrivacyOnly: "Accetto la",
    privacyOnly: "Privacy",
    acceptTermsPrivacy: "Accetta condizioni e privacy",
    addressLabel: "Indirizzo",
    weekdaysLabel: "Giorni",
    timeLabel: "Orario",
    availableLabel: "Disponibile",
    discountLabel: "Sconto",
    minimumOrderLabel: "Ordine minimo",
    typeLabel: "Tipo",
    flatVoucherType: "Voucher sconto forfettario",
    detailsNone: "Nessun ulteriore dettaglio",
    originalBarText: "Testo originale del bar",
    linkCopied: "Link copiato!",
    loginSuccess: "Accesso effettuato!",
    registerSuccess: "Registrazione riuscita!",
    emailPasswordRequired: "Inserisci email e password",
    invalidCredentials: "Credenziali non valide",
    passwordMin8: "Password min. 8 caratteri",
    passwordMismatch: "Le password non coincidono",
  });
  Object.assign(SHOP_TRANSLATIONS.fr, {
    ...
    langItalian: "Italien",
    langFrench: "Français",
    acceptTermsRegister: "J’accepte les",
    andConnector: "et",
    acceptPrivacyOnly: "J’accepte la",
    privacyOnly: "Confidentialité",
    acceptTermsPrivacy: "Veuillez accepter les CGV et la confidentialité",
    ...
    typeLabel: "Type",
    flatVoucherType: "Bon de réduction forfaitaire",
    detailsNone: "Pas d’autres détails",
    originalBarText: "Texte original du bar",
    linkCopied: "Lien copié !",
    loginSuccess: "Connecté !",
    registerSuccess: "Inscription réussie !",
    emailPasswordRequired: "Veuillez saisir l’email et le mot de passe",
    invalidCredentials: "Identifiants invalides",
  ...
  applyShopTranslations = function () {
    ...
    if (regLang && !regLang.value)
      regLang.value =
        _shopLang || localStorage.getItem("barsclusive_lang") || "de";
    var loginNo = document.getElementById("loginNoAccountText");
    if (loginNo) loginNo.textContent = st("noAccountYet");
    var originalLabel = document.getElementById("ddOriginalLabel");
    if (originalLabel) originalLabel.textContent = st("originalBarText");
  };
  ...
  doLogin = async function () {
    ...
    if (!email || !password) {
      showToast(
        st("emailPasswordRequired") || "Bitte Email und Passwort eingeben",
        true,
      );
      return;
    }
    ...
    try {
      ...
      if (r.success) {
        ...
        showToast(
          r.error || st("invalidCredentials") || "Ungültige Zugangsdaten",
          true,
        );
        document.getElementById("loginPassword").value = "";
      }
  ...
  doRegister = async function () {
    ...
    const consent = document.getElementById("regConsent").checked;
    const lang =
      (document.getElementById("regLangSelect") || {}).value ||
      _shopLang ||
      "de";
    if (!name || !email || !password) {
      showToast(st("allFieldsRequired") || "Alle Felder ausfüllen", true);
      return;
    }
    if (password.length < 8) {
      showToast(st("passwordMin8") || "Passwort mind. 8 Zeichen", true);
      return;
    }
    if (password !== passConfirm) {
      showToast(
        st("passwordMismatch") || "Passwörter stimmen nicht überein",
        true,
      );
      return;
    }
    if (!consent) {
      showToast(
        st("acceptTermsPrivacy") || "Bitte AGB & Datenschutz akzeptieren",
        true,
      );
      return;
    }
    try {
      ...
      if (r.success) {
        sessionSet(
          ...
          email,
          "customer",
          (r.customer && r.customer.lang) || lang,
        );
        closeModal("registerModal");
        document.getElementById("regPassword").value = "";
        document.getElementById("regPasswordConfirm").value = "";
        showToast(
          "✅ " + (st("registerSuccess") || "Registrierung erfolgreich!"),
        );
      } else {
  ...
  openDealDetail = function (deal) {
    ...
    if (!modal) return;
    var isPauschal =
      (deal.categories || []).indexOf("pauschalgutscheine") !== -1;
    var imgDiv = document.getElementById("ddImg");
    var existingImg = imgDiv.querySelector("img");
    if (existingImg) existingImg.remove();
    if (deal.image_url) {
    ...
    if (origWrap && (titleDiff || descDiff)) {
      ...
      document.getElementById("ddOriginalLabel").textContent =
        st("originalBarText");
    ...
    if (isPauschal) {
      if (deal.discount_percent)
        info +=
          '<div><span style="color:#999">' +
          escHtml(st("discountLabel")) +
          '</span><span style="color:#FF3366;font-weight:700">' +
          deal.discount_percent +
          "%</span></div>";
      if (deal.min_order)
        info +=
          '<div><span style="color:#999">' +
          escHtml(st("minimumOrderLabel")) +
          "</span><span>" +
          deal.min_order +
          " CHF</span></div>";
      info +=
        '<div><span style="color:#999">' +
        escHtml(st("typeLabel")) +
        "</span><span>" +
        escHtml(st("flatVoucherType")) +
        "</span></div>";
    }
    var weekdays = deal.valid_weekdays || [];
    if (weekdays.length)
    ...
    document.getElementById("ddInfo").innerHTML =
      info ||
      '<div style="color:#666">' + escHtml(st("detailsNone")) + "</div>";
    var shareUrl =
      window.location.origin + window.location.pathname + "?deal=" + deal.id;
    var shareText =
      deal.title +
    ...
    ddCartBtn.textContent =
      "🛒 " + (shopT("cartAddTitle") || "In den Warenkorb");
    ddCartBtn.onclick = function () {
      addToCart(deal);
      closeDealDetail();
    };
  ...
  copyDealLink = function () {
    ...
    navigator.clipboard
      .writeText(url)
      .then(function () {
        showToast(st("linkCopied") || "Link kopiert!");
      })
      .catch(function () {
        showToast((st("linkGenericPrefix") || "Link:") + " " + url);
      });
  };
})();
...
(function () {
  function uiLang() {
    return String(
      _shopLang || shopLang || localStorage.getItem("barsclusive_lang") || "de",
    ).toLowerCase();
  }
  ...
  function timeSlotLabel(slot) {
    ...
    var map = {
      de: {
        morning: "🌅 " + (shopT("morgen") || "Morgen"),
        midday: "☀️ " + (shopT("mittag") || "Mittag"),
        evening: "🌙 " + (shopT("abend") || "Abend"),
      },
      en: { morning: "🌅 Morning", midday: "☀️ Noon", evening: "🌙 Evening" },
      it: {
        morning: "🌅 Mattina",
        midday: "☀️ Mezzogiorno",
        evening: "🌙 Sera",
      },
  ...
  function localizeWeekdayList(raw) {
    ...
    return String(raw)
      .split(",")
      .map(function (part) {
        return translateWeekdayToken(part.trim());
      })
      .join(", ");
  }
  ...
  function statusText(key) {
    ...
    var map = {
      ...
      refund_requested: {
        de: "⏳ Rückerstattung angefordert",
        en: "⏳ Refund requested",
        it: "⏳ Rimborso richiesto",
  ...
  var _origBuildDealCardFinal =
    typeof buildDealCard === "function" ? buildDealCard : null;
      ...
      try {
        card.querySelectorAll("span, div").forEach(function (el) {
          ...
          if (
            ...
            else if (/Mittag/i.test(txt))
              el.textContent = timeSlotLabel("midday");
            else if (/Abend/i.test(txt))
              el.textContent = timeSlotLabel("evening");
          }
        });
      } catch (e) {}
      return card;
  ...
  var _origBuildOrderCardFinal =
    typeof buildOrderCard === "function" ? buildOrderCard : null;
  if (_origBuildOrderCardFinal) {
    buildOrderCard = function (o) {
      ...
      try {
        ...
        card
          .querySelectorAll(".voucher-box > div:first-child")
          .forEach(function (label) {
            label.textContent = shopT("gutscheinCode") || "Voucher code:";
          });

        card
          .querySelectorAll(".voucher-box .share-btn, .voucher-box a")
          .forEach(function (btn) {
            var t = (btn.textContent || "").trim();
            if (/anzeigen|view|visualizza|afficher/i.test(t))
              btn.textContent = "🎫 " + (shopT("gutscheinAnzeigen") || "View");
            else if (
              /link kopieren|copy link|copia link|copier le lien/i.test(t)
            )
              btn.textContent = "📋 " + (shopT("linkKopieren") || "Copy link");
            else if (/teilen|share|condividi|partager/i.test(t))
              btn.textContent = "📱 " + (shopT("shareBtn") || "Share");
          });

        var refundInfo = Array.from(card.querySelectorAll("div")).find(
          function (el) {
            var t = (el.textContent || "").trim();
            return /Rückerstattung angefordert|Refund requested|Rimborso richiesto|Remboursement demandé|Rückerstattet|Refunded|Rimborsato|Remboursé/.test(
              t,
            );
          },
        );
        if (refundInfo) {
          if (o.refund_status === "requested")
            refundInfo.textContent = statusText("refund_requested");
          if (o.refund_status === "completed")
            refundInfo.textContent = statusText("refunded");
        }

        var refundBtn = card.querySelector(".btn-refund");
        if (refundBtn)
          refundBtn.textContent =
            "💰 " + (shopT("refundReq") || "Request refund");
  ...
  var _origOpenDealDetailFinal =
    typeof openDealDetail === "function" ? openDealDetail : null;
      ...
      try {
          ...
          ddInfo.querySelectorAll("div").forEach(function (row) {
            ...
            if (spans.length >= 2) {
              ...
              if (/jours|wochentage|giorni|weekdays/.test(label)) {
                spans[1].textContent = localizeWeekdayList(
                  spans[1].textContent || "",
                );
              }
            }
        ...
        if (shareWrap) {
          shareWrap.querySelectorAll("button").forEach(function (btn) {
            var t = (btn.textContent || "").trim();
            if (/link kopieren|copy link|copia link|copier le lien/i.test(t))
              btn.textContent = "📋 " + (shopT("linkKopieren") || "Copy link");
            if (/teilen|share|condividi|partager/i.test(t))
              btn.textContent = "📱 " + (shopT("shareBtn") || "Share");
          });
        }
      } catch (e) {}
  ...
  function dealsCacheForLang(lang) {
    return "barsclusive_deals_cache_" + lang;
  }
  async function fetchDealsFresh(lang, showErrorsWhenEmpty) {
    ...
    try {
      ...
      var ld2 = document.getElementById("dealsLoading");
      if (ld2) ld2.style.display = "none";
      var dl2 = document.getElementById("dealsList");
      if (dl2) dl2.style.display = "";
      if (!allDeals.length && showErrorsWhenEmpty) {
        var list = document.getElementById("dealsList");
        if (list) {
          list.innerHTML =
            '<div class="empty-state" style="grid-column:1/-1;text-align:center;padding:48px 18px;color:#888"><div style="font-size:38px;margin-bottom:10px">🍸</div><div style="font-size:18px;font-weight:700;margin-bottom:6px">' +
            escHtml(shopT("dealsLoadError") || "Fehler beim Laden der Deals") +
            '</div><div style="font-size:14px">' +
            escHtml(shopT("networkReload") || "Bitte neu laden") +
            "</div></div>";
        }
        showToast(
          shopT("networkReload") || "Verbindungsfehler - bitte neu laden",
  ...
  loadDeals = async function (forceRefresh) {
    ...
    var freshInMemory =
      dealsCache.data &&
      dealsCache.lang === lang &&
      now - Number(dealsCache.timestamp || 0) < CACHE_DURATION;
    ...
    if (!forceRefresh) {
      try {
        var raw =
          localStorage.getItem(dealsCacheForLang(lang)) ||
          localStorage.getItem("barsclusive_deals_cache");
        if (raw) {
          var parsed = JSON.parse(raw);
          if (parsed && parsed.data && parsed.data.length) {
            ...
            attachDistances();
            var ld3 = document.getElementById("dealsLoading");
            if (ld3) ld3.style.display = "none";
            var dl3 = document.getElementById("dealsList");
            if (dl3) dl3.style.display = "";
            renderDeals();
            fetchDealsFresh(lang, false);
            return;
          }
  ...
  var _origSetShopLangFinal =
    typeof setShopLang === "function" ? setShopLang : null;
    ...
    setShopLang = function (lang) {
      ...
      try {
        ...
        if (
          ...
          renderFavorites();
        if (
          typeof _detailDeal !== "undefined" &&
          _detailDeal &&
          document.getElementById("dealDetailModal") &&
          document
            .getElementById("dealDetailModal")
            .classList.contains("active")
        )
          openDealDetail(_detailDeal);
      } catch (e) {}
    };
  }
})();
...
(function () {
  try {
    Object.assign(SHOP_TRANSLATIONS.de, {
      menuTitle: "Mehr entdecken",
      menuNote:
        "Alles Wichtige schnell erreichbar, ohne bis zum Seitenende zu scrollen.",
      customDateBtn: "📅 Datum wählen",
      customDateLabel: "Datum",
      myLocationBtn: "Nach Distanz sortieren",
      geoCurrentLocation: "Aktueller Standort",
      favoritesNav: "Favoriten",
      noAccountYet: "Noch kein Konto?",
      statusPending: "Ausstehend",
      statusBought: "Gekauft",
      statusRedeemed: "Eingelöst",
      timeSlotMorning: "🌅 Morgen",
      timeSlotMidday: "☀️ Mittag",
      timeSlotEvening: "🌙 Abend",
      weekdaysLabel: "Wochentage",
      buyBtn: "Deal kaufen",
      shopBenefitFastTitle: "✨ Direkt sichern",
      shopBenefitFastSub: "In wenigen Sekunden reserviert.",
      shopBenefitLocalTitle: "📍 In deiner Nähe",
      shopBenefitLocalSub: "Bars und Deals nach Distanz entdecken.",
      shopBenefitExclusiveTitle: "🍸 Kuratierte Selektion",
      shopBenefitExclusiveSub: "Breakfast bis Aperitivo an einem Ort.",
      dealsIntro:
        "Kuratierte Deals für Breakfast, Brunch, Aperitivo und Events.",
      cartHeaderBtn: "Warenkorb",
      profitiere: "Jetzt sichern",
    });
    Object.assign(SHOP_TRANSLATIONS.en, {
      menuTitle: "Explore more",
      menuNote:
        "Quick access to the important pages without endless scrolling.",
      customDateBtn: "📅 Pick a date",
      customDateLabel: "Date",
      myLocationBtn: "Sort by distance",
      geoCurrentLocation: "Current location",
      ...
      timeSlotMidday: "☀️ Midday",
      timeSlotEvening: "🌙 Evening",
      weekdaysLabel: "Weekdays",
      buyBtn: "Buy deal",
      shopBenefitFastTitle: "✨ Secure instantly",
      shopBenefitFastSub: "Reserved in just a few seconds.",
      shopBenefitLocalTitle: "📍 Nearby first",
      shopBenefitLocalSub: "Discover bars and deals by distance.",
      shopBenefitExclusiveTitle: "🍸 Curated selection",
      shopBenefitExclusiveSub: "From breakfast to aperitivo in one place.",
      dealsIntro: "Curated deals for breakfast, brunch, aperitivo and events.",
      cartHeaderBtn: "Cart",
      profitiere: "Secure now",
    });
    Object.assign(SHOP_TRANSLATIONS.it, {
      menuTitle: "Scopri di più",
      menuNote:
        "Tutto importante a portata di mano, senza scorrere fino in fondo.",
      customDateBtn: "📅 Scegli data",
      customDateLabel: "Data",
      myLocationBtn: "Ordina per distanza",
      geoCurrentLocation: "Posizione attuale",
      favoritesNav: "Preferiti",
      noAccountYet: "Non hai ancora un account?",
      statusPending: "In attesa",
      statusBought: "Acquistato",
      statusRedeemed: "Riscattato",
      timeSlotMorning: "🌅 Mattina",
      timeSlotMidday: "☀️ Pranzo",
      timeSlotEvening: "🌙 Sera",
      weekdaysLabel: "Giorni",
      buyBtn: "Acquista deal",
      shopBenefitFastTitle: "✨ Blocca subito",
      shopBenefitFastSub: "Prenotato in pochi secondi.",
      shopBenefitLocalTitle: "📍 Vicino a te",
      shopBenefitLocalSub: "Scopri bar e deal per distanza.",
      shopBenefitExclusiveTitle: "🍸 Selezione curata",
      shopBenefitExclusiveSub: "Da breakfast ad aperitivo in un solo posto.",
      dealsIntro: "Deal curati per breakfast, brunch, aperitivo ed eventi.",
      cartHeaderBtn: "Carrello",
      profitiere: "Blocca ora",
    });
    Object.assign(SHOP_TRANSLATIONS.fr, {
      menuTitle: "Découvrir plus",
      menuNote:
        "Toutes les pages importantes rapidement accessibles, sans devoir scroller jusqu’en bas.",
      customDateBtn: "📅 Choisir date",
      customDateLabel: "Date",
      myLocationBtn: "Trier par distance",
      geoCurrentLocation: "Position actuelle",
      favoritesNav: "Favoris",
      noAccountYet: "Pas encore de compte ?",
      statusPending: "En attente",
      statusBought: "Acheté",
      statusRedeemed: "Utilisé",
      timeSlotMorning: "🌅 Matin",
      timeSlotMidday: "☀️ Midi",
      timeSlotEvening: "🌙 Soir",
      weekdaysLabel: "Jours",
      buyBtn: "Acheter l’offre",
      shopBenefitFastTitle: "✨ Réserver vite",
      shopBenefitFastSub: "Réservé en quelques secondes.",
      shopBenefitLocalTitle: "📍 Près de toi",
      shopBenefitLocalSub: "Découvre bars et deals par distance.",
      shopBenefitExclusiveTitle: "🍸 Sélection soignée",
      shopBenefitExclusiveSub: "Du breakfast à l’aperitivo au même endroit.",
      dealsIntro:
        "Des deals sélectionnés pour breakfast, brunch, aperitivo et événements.",
      cartHeaderBtn: "Panier",
      profitiere: "Réserver",
    });
  } catch (e) {}
    ...
    if (input && input.value) {
      ...
      if (!isNaN(dt.getTime())) {
        try {
          el.textContent =
            "📅 " +
            dt.toLocaleDateString(document.documentElement.lang || "de-CH", {
              day: "2-digit",
              month: "2-digit",
              year: "numeric",
            });
  ...
  function updateShopUserUi() {
    ...
    if (userBtn) {
      userBtn.textContent = s ? "👤 " + (s.name || "") : "👤 " + st("loginBtn");
      userBtn.setAttribute("aria-expanded", "false");
    }
    if (btnOrders) btnOrders.style.display = s ? "inline-flex" : "none";
    if (btnFavorites) btnFavorites.style.display = s ? "inline-flex" : "none";
    if (drawerFav) drawerFav.style.display = s ? "flex" : "none";
    if (drawerOrders) drawerOrders.style.display = s ? "flex" : "none";
    if (btnOrders) btnOrders.textContent = "📦 " + st("orders");
    ...
    if (!s && userDropdown) userDropdown.classList.remove("show");
    var noAcc = document.getElementById("loginNoAccountText");
    if (noAcc) noAcc.textContent = st("noAccountYet") || noAcc.textContent;
    var regLink = document.getElementById("linkToRegister");
    if (regLink)
      regLink.textContent = st("registrieren") || regLink.textContent;
  }
  ...
  applyShopTranslations = function () {
    ...
    var input = document.getElementById("locationInput");
    if (input) input.placeholder = st("searchPLZ");
    var search = document.getElementById("shopSearch");
    if (search) search.placeholder = st("searchBarDeal");
  };

  var _origSessionSet = sessionSet;
  sessionSet = function (token, name, email, role) {
    _origSessionSet(token, name, email, role);
    updateShopUserUi();
  };
  ...
  requestGeoPermission = function () {
    ...
    navigator.geolocation.getCurrentPosition(
      function (pos) {
        ...
        dismissGeoBanner();
        showToast(
          shopT("dealsSortedByDistance") ||
            "📍 Deals werden nach Nähe sortiert",
        );
        sortDealsByDistance();
      },
      function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (menuBtn) menuBtn.addEventListener("click", openShopDrawer);
    var drawerClose = document.getElementById("shopDrawerClose");
    if (drawerClose) drawerClose.addEventListener("click", closeShopDrawer);
    var drawerOverlay = document.getElementById("shopDrawerOverlay");
    if (drawerOverlay) drawerOverlay.addEventListener("click", closeShopDrawer);
    var drawerDeals = document.getElementById("drawerDealsBtn");
    if (drawerDeals)
      drawerDeals.addEventListener("click", function () {
        closeShopDrawer();
        showView("deals");
      });
    var drawerFav = document.getElementById("drawerFavoritesBtn");
    if (drawerFav)
      drawerFav.addEventListener("click", function () {
        closeShopDrawer();
        showView("favorites");
      });
    var drawerOrders = document.getElementById("drawerOrdersBtn");
    ...
    if (customDate)
      customDate.addEventListener("change", updateCustomDateLabel);
    updateCustomDateLabel();
    updateShopUserUi();
  });
})();
