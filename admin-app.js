...
function renderBars(bars) {
  ...
  bars.forEach(function(b) {
    ...
    var mwstInfo = b.mwst_liable ? ('\u2705 ' + esc(b.mwst_number || '-')) : '\u274C Nicht MWST-pflichtig';
    detailTd.innerHTML='<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;font-size:13px"><div><strong style="color:#FF3366">Kontakt & Finanzen</strong><p style="margin:6px 0;color:#ccc">\uD83D\uDCCD '+esc(b.address||'-')+', '+esc(b.zip||'')+' '+esc(b.city||'')+'</p><p style="margin:6px 0;color:#ccc">\uD83D\uDCE7 '+esc(b.email||'-')+'</p><p style="margin:6px 0;color:#ccc">\uD83D\uDCDE '+esc(b.phone||'-')+'</p><p style="margin:6px 0;color:#ccc">\uD83C\uDFE6 IBAN: <strong style="color:#fff;font-family:monospace">'+esc(b.iban||'-')+'</strong></p><p style="margin:6px 0;color:#ccc">\uD83D\uDCF1 Twint: '+esc(b.twint||'-')+'</p><p style="margin:6px 0;color:#ccc">\uD83E\uDDFE MWST: '+mwstInfo+'</p><p style="margin:6px 0;color:#999;font-size:11px">Registriert: '+fmtDate(b.created_at)+'</p>'+(b.latitude&&b.longitude?'<p style="margin:6px 0;color:#888;font-size:11px">\uD83D\uDCCC '+esc(b.latitude)+', '+esc(b.longitude)+'</p>':'')+'</div><div id="barStats_'+b.id+'"><strong style="color:#FF3366">Laden...</strong></div></div>';
